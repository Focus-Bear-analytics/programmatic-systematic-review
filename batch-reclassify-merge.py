"""Merge Bedrock batch reclassification results back into the combined dataset.
Downloads each job's .jsonl.out from S3, extracts the model's JSON classification,
coerces to the enum, applies deterministic rule overrides, and writes
RaterA_ (Nova) / RaterB_ (Sonnet) + Consensus_ for every survivor. Leaves
Final_/Human_/Adjudicated untouched. Backs up first. Run with the UI stopped.
"""
...
jobs = json.load(open("/tmp/batch_jobs.json"))


def out_text(model_name, mo):
    if model_name == "sonnet":  # Anthropic InvokeModel response
...
def main():
    nova = load_results("nova"); sonnet = load_results("sonnet")
    print(f"parsed nova={len(nova)} sonnet={len(sonnet)}")
    df = pd.read_csv(WORK, dtype=str).fillna("")
    shutil.copy(WORK, WORK.replace(".csv", "_pre_batch_backup.csv"))
    ...
    for idx in set(nova) & set(sonnet):
        ...
        df.at[idx, "RaterA_model"] = "Nova 2 Lite"
        df.at[idx, "RaterB_model"] = "Claude Sonnet 4.6"
        df.at[idx, "Consensus_Reached"] = "Y" if all(nzc(a.get(f)) == nzc(b.get(f)) for f in CONSENSUS_FIELDS) else "N"
        updated += 1
    df.to_csv(WORK, index=False)
    print(f"merged {updated} survivors -> {WORK} (backup: _pre_batch_backup.csv)")
