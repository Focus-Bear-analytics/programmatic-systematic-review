...
- intervention_type: one of {ALLOWED['intervention_type']}.
  ...
  Otherwise choose by this precedence (first that applies): vr_ar (immersive virtual reality via a headset or CAVE, or an augmented-reality system) > video_modeling (the intervention is watching video demonstrations/models of target behaviours or skills — including video self-modeling — to observe and imitate; passive video content even if shown on a device) > mobile_app (smartphone/tablet app) > web_app (browser/online program or website) > wearable (body-worn device) > telecoaching (remote human coaching) > telecounselling (remote therapy/counselling) > software_tool (other standalone/desktop software with no app or web delivery) > non_software_based (a real intervention with NO digital component — e.g. face-to-face therapy, medication, in-person coaching, a printed workbook, an exercise programme).
